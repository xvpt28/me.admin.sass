...
public class ExpenseService(ExpenseRepository expenseRepository, AuthService authService, IOptions<FileSetting> fileSetting)
{
	...
	public async Task<BaseResponse<string>> CreateExpenseRecord(string outletId, CreateExpenseRecordRequestDto request)
	{
		try
		{
	...
	public async Task<BaseResponse<bool>> UpdateExpenseRecord(string expenseId, UpdateExpenseRecordRequestDto request)
	{
		try
		{
			var userId = _authService.GetUserId();
			...
			if (response == null) throw new Exception("Record is not found");
			var currentTimestamp = DateTimeOffset.Now.ToUnixTimeMilliseconds();
	...
	public async Task<BaseResponse<bool>> DeleteExpenseRecord(string expenseId)
	{
		try
		{
			...
			if (userId == null) throw new Exception("Invalid user");
			var response = await _expenseRepository.GetById(expenseId);
			if (response == null) throw new Exception("Expense record is not found");
			// todo: delete file 
	...
	public async Task<BaseResponse<string>> UploadImage(IFormFile file)
	{
		try
		{
			if (file.Length == 0) throw new Exception("Invalid file");
	...
	public BaseResponse<bool> DeleteImage(string filePath)
	{
		try
		{
			var absPath = Path.Combine(_fileSetting.RootFolder, _fileSetting.ExpenseRootFolder, filePath);

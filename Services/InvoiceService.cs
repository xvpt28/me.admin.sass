...
)
{
    ...
    public async Task<BaseResponse<List<GetAllInvoiceResponseDto>>> GetAllInvoiceRecord(
        string outletId,
        GetInvoiceFilterDto filter
    )
    {
        try
        {
        ...
        catch (Exception e)
        {
            ...
            return new BaseResponse<List<GetAllInvoiceResponseDto>>
            {
                Success = false,
                Message = e.Message,
            };
    ...
    public async Task<BaseResponse<string>> GenerateInvoice(string orderId, CreateInvoiceDto body)
    {
        try
        {
            ...
            if (resp != null)
            {
                if (resp.FilePath != null && File.Exists(resp.FilePath))
                    File.Delete(resp.FilePath);
            ...
            var filePath = GenerateInvoiceFile(
                order,
                orderItem,
                outlet,
                entity,
    ...
    )
    {
        ...
        var fileName =
            data.Type == "invoice"
                ? $"ME-INV-{formattedInvoiceId}.pdf"
                : $"ME-QUO-{formattedInvoiceId}.pdf";
        ...
        if (
            !File.Exists(fontPathRegular)
            || !File.Exists(fontPathBold)
            || !File.Exists(fontPathSemiBold)
        )
            throw new Exception("Font file not found");
        var baseFontNormal = BaseFont.CreateFont(
            fontPathRegular,
            BaseFont.IDENTITY_H,
            BaseFont.EMBEDDED
        );
        var baseFontSemiBold = BaseFont.CreateFont(
            fontPathSemiBold,
            BaseFont.IDENTITY_H,
            BaseFont.EMBEDDED
        );
        var baseFontBold = BaseFont.CreateFont(
            fontPathBold,
            BaseFont.IDENTITY_H,
            BaseFont.EMBEDDED
        );
        ...
        // business info
        var issueDate = DateTimeOffset
            .FromUnixTimeMilliseconds(data.IssuedDate)
            .ToLocalTime()
            .DateTime;
        ...
        if (!payload.HideDiscount)
        {
            ...
            var tableSubTotal = new List<string>
            {
                "",
                "",
                "Sub Total",
        ...
        if ((payload.Discount is > 0 || order.Discount is > 0) && !payload.HideDiscount)
        {
            ...
            var tableDiscount = new List<string>
            {
                "",
                "",
                "Discount",
                "$" + discount.ToString("N2", CultureInfo.InvariantCulture),
            };
            index = 0;
        ...
        var tableGrandTotal = new List<string>
        {
            "",
            "",
            "Grand Total",
        ...
        if (data.Type != "quotation")
        {
            ...
            if (order.UpdatedAt > 0)
            {
                var paymentDate = DateTimeOffset
                    .FromUnixTimeMilliseconds(order.UpdatedAt)
                    .ToLocalTime()
                    .DateTime;
                var paymentDateFormatted = paymentDate.ToString(
                    "dd MMM yyyy",
        ...
        if (
            !string.IsNullOrEmpty(data.BilledTo)
            || !string.IsNullOrEmpty(data.BilledCompanyAddress)
            || !string.IsNullOrEmpty(data.BilledCompanyUEN)
        )
        {
        ...
        if (!File.Exists(stampPath))
            throw new Exception("Stamp file not found");
        ...
        writer.PageEvent = new Footer(
            $"{outlet.OutletAddress}, {outlet.OutletPostcode}",
            outlet.OutletPhoneNumber,
            textFooter
        );
...
public class Footer(string location, string contact, Font font) : PdfPageEventHelper
{
    ...
    {
        var info = new List<string> { _location, _contact, "https://www.mecafegames.com" };

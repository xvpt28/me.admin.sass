...
public class ImportanceService(ImportanceRepository importanceRepository, AuthService authService)
{
	...
	public async Task<BaseResponse<bool>> UpdateImportanceById(string noteId, UpdateImportanceDto request)
	{
		try
		{
			var importance = await _importanceRepository.GetById(noteId);
			if (importance == null) return new BaseResponse<bool> { Success = false, Message = "Importance not found" };
	...
	public async Task<BaseResponse<bool>> CreateImportanceById(string outletId, CreateImportanceDto request)
	{
		try
		{

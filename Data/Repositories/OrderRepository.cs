...
public class OrderRepository(AppDbContext appDbContext)
{
    ...
    public async Task<List<Order>> GetAllOngoingOrder(string outletId)
    {
        ...
        var tblOrder = db.GetTable<Order>()
            .Where(x =>
                x.DeletedAt == null && x.OutletId == outletId && x.OrderStatus == "ongoing"
            );
    ...
    public async Task<List<Order>> GetAllOrdersWithFilters(
        string outletId,
        GetOrderFilterDto filter
    )
    {
        ...
        {
            query = query.Where(x =>
                x.CreatedAt >= filter.StartTimestamp && x.CreatedAt <= filter.EndTimestamp
            );
        }
    ...
    public async Task<List<Order>> GetAllOrderPayByCashByDate(
        string outletId,
        long startTimestamp,
        long endTimestamp
    )
    {
        ...
        return await db.GetTable<Order>()
            .Where(x =>
                ...
                && x.CreatedAt >= startTimestamp
                && x.CreatedAt <= endTimestamp
            )
            .ToListAsync();
    }

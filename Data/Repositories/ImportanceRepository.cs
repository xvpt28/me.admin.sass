...
public class ImportanceRepository(AppDbContext appDbContext)
{
	...
	public async Task<List<Importance>> GetAll(string outletId)
	{
		...
		var tblImp = db.GetTable<Importance>().Where(x => x.DeletedAt == null && x.OutletId == outletId && x.IsArchived == false).OrderBy(x => x.CreatedAt);
		var tblUser = db.GetTable<User>();
		var query = from imp in tblImp
	...
	{
		await using var db = _appDbContext.GetDatabase();
		return await db.GetTable<Importance>().Where(x => x.DeletedAt == null && x.NoteId == id).FirstOrDefaultAsync();
	}

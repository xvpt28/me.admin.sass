...
public class DailyRecordRepository(AppDbContext appDbContext)
{
    ...
    public async Task<List<GetDailyRecordResponseDto>> GetAll(string outletId)
    {
        ...
        var tblDr = db.GetTable<DailyRecord>()
    ...
    public async Task<List<GetDailyRecordWithFilterDataDto>> GetAllDailyWithFilter(
        string outletId,
        GetDailyRecordFilterDto filter
    )
    {
        await using var db = _appDbContext.GetDatabase();
        var tblDr = db.GetTable<DailyRecord>()
    ...
    {
        await using var db = _appDbContext.GetDatabase();
        return await db.GetTable<DailyRecord>()
            .Where(x => x.DeletedAt == null && x.OutletId == outletId && x.RecordDate < date)
    ...
    public async Task<DailyRecord?> GetById(string id)
    {
        await using var db = _appDbContext.GetDatabase();
        return await db.GetTable<DailyRecord>()
            .Where(x => x.DeletedAt == null && x.RecordId == id)
            .FirstOrDefaultAsync();
    }
    ...
    public async Task<List<GetDailyRecordResponseDto>> GetAllByDate(long date)
    {
        await using var db = _appDbContext.GetDatabase();
        var tblDr = db.GetTable<DailyRecord>()
            .Where(x => x.RecordDate == date && x.DeletedAt == null);
        var tblUser = db.GetTable<User>();
        var tblOutlet = db.GetTable<Outlet>();
        var query =
            from dr in tblDr

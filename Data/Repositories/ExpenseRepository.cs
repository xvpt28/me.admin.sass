...
public class ExpenseRepository(AppDbContext appDbContext)
{
	...
	public async Task<List<Expense>> GetAllByOutletAndDate(string outletId, long startDate, long endDate)
	{
		await using var db = _appDbContext.GetDatabase();
		return await db.GetTable<Expense>().Where(x =>
				x.DeletedAt == null &&
				x.OutletId == outletId &&
	...
	{
		await using var db = _appDbContext.GetDatabase();
		return await db.GetTable<Expense>().Where(x => x.DeletedAt == null && x.ExpenseId == id).FirstOrDefaultAsync();
	}

...
public class OutletRepository(AppDbContext appDbContext) : IRepository<Outlet>
{
	...
	{
		await using var db = _appDbContext.GetDatabase();
		return await db.GetTable<Outlet>().Where(x => x.OutletId == outletId && x.DeletedAt == null).FirstOrDefaultAsync();
	}
	...
	{
		await using var db = _appDbContext.GetDatabase();
		return await db.GetTable<Outlet>().Where(x => x.OutletName == outletName && x.DeletedAt == null).FirstOrDefaultAsync();
	}
}
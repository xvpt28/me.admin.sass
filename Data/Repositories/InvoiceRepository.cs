...
public class InvoiceRepository(AppDbContext appDbContext)
{
    ...
    public async Task<List<GetAllInvoiceResponseDto>> GetAllWithFilter(
        string outletId,
    ...
    )
    {
        ...
        if (filter.StartDate != null && filter.EndDate != null)
        {
            query =
                from i in query
                where i.CreatedAt >= filter.StartDate && i.CreatedAt <= filter.EndDate
                select i;
        }
        ...
        if (filter.Search != null && filter.Search.Trim() != string.Empty)
        {
            ...
            query = query.Where(x =>
                x.InvoiceId.ToString().Trim().Contains(search)
    ...
    {
        await using var db = _appDbContext.GetDatabase();
        return await db.GetTable<Invoice>()
            .Where(x => x.DeletedAt == null && x.InvoiceId == invoiceId)
            .FirstOrDefaultAsync();
    }
    ...
    {
        await using var db = _appDbContext.GetDatabase();
        var invoice = await db.GetTable<Invoice>()

...
public class OrderItemRepository(AppDbContext appDbContext)
{
	...
	public async Task<List<OrderItem>> GetAll(string orderId)
	{
		...
		return await db.GetTable<OrderItem>().Where(x => x.DeletedAt == null && x.OrderId == orderId).OrderBy(x => x.CreatedAt).ToListAsync();
	}
	...
	public async Task<List<OrderItemWithMenu>> GetAllWithMenu(string orderId)
	{
		...
		var query = from o in tblOrderItem
	...
	{
		await using var db = _appDbContext.GetDatabase();
		return await db.GetTable<OrderItem>().Where(x => x.DeletedAt == null && x.OrderItemId == id).FirstOrDefaultAsync();
	}
